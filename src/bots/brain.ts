...
export class BotDirector {
  ...
  private planPlate(s: SimState, bot: Chef, plate: Plate, find: Finder, findAny: Finder): Job | null {
    ...
    // `=== 0`, not `!...length` -- see the note in sim.ts planGrab. TSTL emits
    // `not #t`, which is FALSE in Lua for an empty table, so this inverted on
    // Roblox and bots never returned a spare clean plate to the rack.
    if (plate.contents.length === 0 && !plate.dirty) {
